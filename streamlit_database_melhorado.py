...
def display_scenario_summary(scenario: Dict, index: int):
    """Exibe um resumo colapsável do cenário"""
    ...
    # Toggle do estado
    if st.button("", key=f"toggle_{scenario_id}", help="Clique para expandir/recolher"):
    ...
    with st.container():
        ...
        <div class="scenario-summary {priority_class} {'expanded' if is_expanded else ''}">
            <div class="scenario-title">
                {scenario.get('nome', 'Sem nome')}
                <span class="expand-icon {'rotated' if is_expanded else ''}">▼</span>
            </div>
            ...
            <div class="scenario-description">
                {scenario.get('descricao', 'Sem descrição')}
            </div>
        </div>
        """, unsafe_allow_html=True)

        # Palavras-chave (sempre visíveis)
        keywords = scenario.get('keywords', [])
        if keywords:
            st.markdown('<div class="keywords-container">',
                        unsafe_allow_html=True)
            for keyword in keywords:
                st.markdown(
                    f'<span class="keyword-tag">{keyword}</span>', unsafe_allow_html=True)
...
def main():
    ...
    if selected_area:
        ...
        # Exibir cenários com interface colapsável
        if scenarios:
